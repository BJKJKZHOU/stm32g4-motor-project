...
在中心对齐PWM模式下，0时刻和ARR时刻的下管导通状态不同：
...
| 扇区 | 0时刻采样 | ARR时刻采样 | 计算相 |
|------|----------|------------|--------|
| 1 | Ib, Ic | Ia, Ib | Ic = -(Ia+Ib) |
| 2 | Ia, Ic | Ib, Ic | Ia = -(Ib+Ic) |
| 3 | Ia, Ic | Ib, Ic | Ia = -(Ib+Ic) |
| 4 | Ia, Ib | Ib, Ic | Ic = -(Ia+Ib) |
| 5 | Ia, Ib | Ia, Ic | Ib = -(Ia+Ic) |
| 6 | Ib, Ic | Ia, Ic | Ib = -(Ia+Ic) |

**原理说明**：
- 在0时刻：计数器从0开始上升，某些相的下管刚开始导通
- 在ARR时刻：计数器到达峰值，下管导通状态与0时刻互补
- 通过两次采样，确保至少能采集到两相真实电流
                        ...
                        float* ia, float* ib, float* ic)
{
    ...
    switch(sector) {
        ...
        case 2:  // Sector 2: 0时刻采Ia,Ic，ARR时刻采Ib,Ic
            i_temp[0] = -(i_temp[1] + i_temp[2]);              // Ia计算
            Current_ADCToPhysical(adc_at_arr[1], &i_temp[1]);  // Ib from ARR
            Current_ADCToPhysical(adc_at_zero[2], &i_temp[2]); // Ic from 0
            break;
        ...
        case 5:  // Sector 5: 0时刻采Ia,Ib，ARR时刻采Ia,Ic
            Current_ADCToPhysical(adc_at_arr[0], &i_temp[0]);  // Ia from ARR
            i_temp[1] = -(i_temp[0] + i_temp[2]);              // Ib计算
            Current_ADCToPhysical(adc_at_zero[2], &i_temp[2]); // Ic from 0
            break;
