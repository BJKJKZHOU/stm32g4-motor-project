...
#### 1. 数据结构扩展
- [x] 在 `Motor_LimitParams_t` 结构体中添加位置限位参数
  - `position_limit_enable` (bool) - 位置限位使能标志
  - `position_limit_min` (float) - 最小位置限值 (rad，机械角度)
  - `position_limit_max` (float) - 最大位置限值 (rad，机械角度)
- [x] 默认不启用位置限位（`position_limit_enable = false`）

#### 2. HMI 参数编号系统
- [x] 定义限值参数数量常量 `LIMIT_PARAM_COUNT = 6`
- [x] 创建限值参数描述数组 `limit_param_descs[]`
- [x] 创建限值参数映射数组 `limit_param_maps[][]`

**HMI 编号映射表**：

| HMI 编号 | 参数名 | 单位 | 描述 | 类型 | 可写 |
|---------|--------|------|------|------|------|
| P2001 | I_limit_user | A | 用户最大电流限值 | float | 是 |
| P2002 | speed_limit_user | rpm | 用户最大转速限值 | float | 是 |
| P2003 | position_limit_enable | - | 位置限位使能 | bool | 是 |
| P2004 | position_limit_min | rad | 最小位置限值 | float | 是 |
| P2005 | position_limit_max | rad | 最大位置限值 | float | 是 |
| P2006 | I_limit_max | A | 硬件最大电流限值 | float | 否（只读） |

#### 3. 核心函数实现
- [x] `MotorParams_SetLimitParam()` - 设置限值参数
  - 支持 HMI 编号（P2001-P2006）和参数名两种方式
  - 自动处理 bool 类型参数（P2003）
  - 只读参数保护（P2006 不可修改）
  - 自动更新实际限值（I_limit_actual、speed_limit_actual）
  - 位置限值同时显示 rad 和度数

- [x] `MotorParams_PrintLimits()` - 打印限值参数
  - 显示 HMI 编号、参数名、用户限值、硬件限值、实际限值
  - 位置限位同时显示 rad 和度数
  - 只读参数标注

#### 4. 命令系统集成
- [x] 新增 `limit` 命令 - 显示限值参数
  - `limit` - 显示激活电机的限值参数
  - `limit 0` - 显示电机 0 的限值参数
  - `limit motor1` - 显示电机 1 的限值参数

- [x] 扩展 `set` 命令 - 支持限值参数设置
  - 支持 HMI 编号：`set motor0 P2001 = 10.0`
  - 支持参数名：`set motor0 I_limit_user = 10.0`
  - 支持 bool 类型：`set motor0 P2003 = 1` 或 `set motor0 position_limit_enable = true`

#### 5. 文档更新
- [x] 更新 command.h 文档说明新增命令格式
- [x] 提供详细的使用示例和命令对比
...
#### 1. 限位模式扩展
- [ ] 添加限位模式枚举：区分角度限位和直线限位
  ```c
  typedef enum {
      LIMIT_MODE_DISABLED = 0,  // 禁用限位
      LIMIT_MODE_ANGLE,         // 角度限位（rad）
      LIMIT_MODE_LINEAR         // 直线限位（mm，需要电子齿轮比）
  } PositionLimitMode_t;
  ```

#### 2. 电子齿轮比支持
- [ ] 添加电子齿轮比参数到 `Motor_LimitParams_t`
  ```c
  float gear_ratio;  // 电子齿轮比 (mm/rev)，将机械圈数映射到直线位移
  ```
- [ ] 实现单位转换函数
  - `Position_RadToLinear()` - 角度转直线位移
  - `Position_LinearToRad()` - 直线位移转角度

#### 3. 限位检查函数
- [ ] 实现 `MotorParams_CheckPositionLimit()` 函数
  - 在位置控制循环中调用
  - 检查当前位置是否超出限位范围
  - 返回限位状态（正常/触发最小限位/触发最大限位）
  ```c
  typedef enum {
      LIMIT_STATUS_OK = 0,
      LIMIT_STATUS_MIN_EXCEEDED,
      LIMIT_STATUS_MAX_EXCEEDED
  } LimitStatus_t;

  LimitStatus_t MotorParams_CheckPositionLimit(uint8_t motor_id, float current_position);
  ```

#### 4. 限位报警机制
- [ ] 添加限位报警标志到 `Motor_LimitParams_t`
  ```c
  bool position_limit_alarm;  // 位置限位报警标志
  ```
- [ ] 实现限位触发处理
  - 触发限位时快速停止电机（不考虑减速度）
  - 设置报警标志
...
- [ ] 添加位置模式枚举
  ```c
  typedef enum {
      POSITION_MODE_ABSOLUTE = 0,  // 绝对位置
      POSITION_MODE_INCREMENTAL    // 增量位置
  } PositionMode_t;
  ```
- [ ] 实现位置模式切换和计算
...
// 频率配置
#define CURRENT_LOOP_FREQ     16000   // 16kHz电流环
