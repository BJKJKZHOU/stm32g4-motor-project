...
static float compute_scurve_velocity(const VelocityMode_t *vm, float t) {
    if (vm->phase == 0) {
        ...
        float t4 = t3 * t_norm;
        float t5 = t4 * t_norm;
        // 使用 decel_start_velocity 作为目标速度
        return vm->decel_start_velocity * (10.0f * t3 - 15.0f * t4 + 6.0f * t5);
    } else if (vm->phase == 1) {
        // 匀速段
        return vm->target_velocity;
    } else if (vm->phase == 2) {
        ...
        float t4 = t3 * t_norm;
        float t5 = t4 * t_norm;
        float v = vm->decel_start_velocity * (1.0f - (10.0f * t3 - 15.0f * t4 + 6.0f * t5));
        return clamp(v, 0.0f, vm->decel_start_velocity);
    }
...
float VelocityMode_Run(VelocityMode_t *vm, float dt, bool stop_request) {
    ...
    // 处理停止请求（持续模式）
    if (vm->run_time == 0.0f && stop_request && vm->phase < 2) {
        // 保存当前速度作为减速起始速度
        vm->decel_start_velocity = vm->current_velocity;
        // 重新计算减速时间（从当前速度减速到0）
        vm->decel_time = vm->decel_start_velocity / vm->deceleration;
        // 切换到减速阶段
        vm->phase = 2;
        vm->elapsed_time = 0.0f;
        // 注意：不要在这里累加 dt，让后续逻辑统一处理
    }
